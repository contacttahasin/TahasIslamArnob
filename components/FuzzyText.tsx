  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      if (gradient && Array.isArray(gradient) && gradient.length >= 2) {
        ...
        // Canvas fillStyle silently ignores 'currentColor'/'inherit' (not
        // valid CSS <color> values per the setter's parsing algorithm) and
        // keeps its prior value, i.e. the 2D context default of solid
        // black — resolve those keywords the same way fontFamily does.
        offCtx.fillStyle =
          color === 'currentColor' || color === 'inherit' ? window.getComputedStyle(canvas).color : color;
      }
